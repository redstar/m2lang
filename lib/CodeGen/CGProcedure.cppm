//===--- CGProcedure.cppm - Code Generator for Procedures -----------------===//
//
...
export module m2lang.codegen:CGProcedure;
...
private:
  llvm::LLVMContext &getContext() { return CGU.getLLVMCtx(); }
...
public:
  CGProcedure(CGUnit &CGU) : CGU(CGU), Builder(CGU.getLLVMCtx()) {}
  void run(Procedure *Proc);
  void run(const Block &Block, const llvm::Twine &Name);
};
} // namespace m2lang
...
llvm::Value *CGProcedure::readLocalVariableRecursive(llvm::BasicBlock *BB,
  ...
  if (!CurrentDef[BB].Sealed) {
    ...
    llvm::PHINode *Phi =
        BB->empty() ? llvm::PHINode::Create(mapType(Decl), 0, "", BB)
...
llvm::Value *CGProcedure::addPhiOperands(llvm::BasicBlock *BB,
                                         Declaration *Decl,
                                         llvm::PHINode *Phi) {
...
void CGProcedure::writeVariable(llvm::BasicBlock *BB, Declaration *Decl,
  ...
  if (auto *V = llvm::dyn_cast<Variable>(Decl)) {
    if (V->getStorage() == Variable::Stack)
      ...
      auto *Inst = Builder.CreateStore(Val, CGU.getGlobal(Decl));
      CGU.decorateInst(Inst, V->getTypeDenoter());
    } else
    ...
    if (FP->isCallByReference()) {
      ...
      CGU.decorateInst(Inst, FP->getType());
    } else
      writeLocalVariable(BB, Decl, Val);
  } else
...
llvm::Type *CGProcedure::mapType(Declaration *Decl, bool HonorReference) {
  ...
  if (auto *FP = llvm::dyn_cast<FormalParameter>(Decl)) {
    if (FP->isCallByReference() && HonorReference)
      return CGU.PtrTy;
    return CGU.convertType(FP->getType());
  }
  if (auto *T = llvm::dyn_cast<Type>(Decl))
    return CGU.convertType(T);
  return CGU.convertType(llvm::cast<Variable>(Decl)->getTypeDenoter());
}

llvm::FunctionType *CGProcedure::createFunctionType(Procedure *Proc) {
  llvm::Type *ResultTy = CGU.VoidTy;
...
llvm::Function *CGProcedure::createFunction(Procedure *Proc,
  ...
  llvm::Function *Fn =
      llvm::Function::Create(Fty, llvm::GlobalValue::ExternalLinkage,
                             utils::mangleName(Proc), CGU.getModule());
  // Give parameters a name.
  for (auto Pair : llvm::enumerate(Fn->args())) {
    llvm::Argument *Arg = &Pair.value();
    FormalParameter *FP = Proc->getParams()[Pair.index()];
    if (FP->isCallByReference()) {
      llvm::AttrBuilder Attr(CGU.getLLVMCtx());
      auto Sz = CGU.getModule()->getDataLayout().getTypeStoreSize(
          CGU.convertType(FP->getType()));
...
llvm::Value *CGProcedure::emitIntLiteral(IntegerLiteral *IntLit) {
  return llvm::ConstantInt::get(CGU.Int64Ty, IntLit->getValue());
}

llvm::Value *CGProcedure::emitRealLiteral(RealLiteral *RealLit) {
  return llvm::ConstantFP::get(CGU.getLLVMCtx(), RealLit->getValue());
}
...
llvm::Value *CGProcedure::emitBoolLiteral(BooleanLiteral *BoolLit) {
  return llvm::ConstantInt::getBool(CGU.Int8Ty, BoolLit->isValue());
}

llvm::Value *CGProcedure::emitNilValue(NilValue *E) {
  return llvm::Constant::getNullValue(CGU.Int8Ty->getPointerTo());
}
  ...
  if (llvm::isa_and_nonnull<Variable>(Decl) ||
    ...
    for (auto *I = Selectors.begin(), *E = Selectors.end(); I != E;) {
      if (auto *IdxSel = llvm::dyn_cast<IndexSelector>(*I)) {
        ...
        Val = Builder.CreateLoad(CGU.convertType(TyDe), Val);
...
void CGProcedure::emitIf(IfStatement *Stmt) {
  ...
  llvm::BasicBlock *ElseBB =
      HasElse ? llvm::BasicBlock::Create(CGU.getLLVMCtx(), "else.body", Fn)
              : nullptr;
...
void CGProcedure::run(const Block &Block, const llvm::Twine &Name) {
  Fty = llvm::FunctionType::get(CGU.VoidTy, {}, /* IsVarArgs */ false);
